...
namespace SecIotAgent.WEB
{
    ...
    {
        Logs commonV2 = new Logs();
        Central_ProcessingClass central_ProcessingClass = new Central_ProcessingClass();
        string strLogMsg = string.Empty;
        ...
        private bool ProcessClientRequest(int dataSize)
        {
            ...
            if (mask)
            {
                ...
                strLogMsg = "response :" + Encoding.UTF8.GetString(decoded);
                commonV2.Log_info(strLogMsg, (int)LOGINFO.INFO, (int)LOGINFO_FUNCTION.WEBSOCKET);
                string strDecoded = Encoding.UTF8.GetString(decoded);
                ...
                if (ValidateJSON(strDecoded) == true)
                {
                    var obj = JObject.Parse(strDecoded);
                    ...
                    if (!String.IsNullOrEmpty(cmd))
                    {
                        ...
                        {
                            isJsonRequest = true;
                            nCmdType = (int)WEBSOCK_CMD.CHECK;
                        }
                        else if (String.Compare(cmd, "login", false) == 0)
                        {
                            isJsonRequest = true;
                            nCmdType = (int)WEBSOCK_CMD.LOGIN;
                        }
                        else if (String.Compare(cmd, "exchange_media_key", false) == 0)
                        {
                            isJsonRequest = true;
                            nCmdType = (int)WEBSOCK_CMD.EXCH_MEDIA_KEY;
                        }
                    }
                    // 메시지 수신에 대한 처리, 반환 값은 연결 종료 여부
                }

                bool isWebsocketStatus = 0;
                string strWebServerStatus = string.Empty;
                byte[] TokenData = null;
                    ...
                    case PayloadDataType.Text:
                        {
                            if (isJsonRequest == true)
                            {
                                ...
                                switch((WEBSOCK_CMD)nCmdType)
                                {
                                    case WEBSOCK_CMD.CHECK:
                                        {
                                            isWebsocketStatus = central_ProcessingClass.GetSocketStatus();
                                            strWebServerStatus = isWebsocketStatus ? "is_alive" : "isnot_alive";
                                            SendMsg = String.Format("{ \"cmd\" : \"check\",\"result\":\"ok\",\"status\":\"{0}\"}", strWebServerStatus);
                                        }
                                        break;
                                    case WEBSOCK_CMD.LOGIN:
                                        {
                                            TokenData = central_ProcessingClass.RequestServerAuthentication(UserId, UserPW);
                                            SendMsg = "{ \"cmd\" : \"login\",\"result\":\"ok\",\"status\":\"is_success\"}";
                                        }
                                        break;
                                    case WEBSOCK_CMD.EXCH_MEDIA_KEY:
                                        {
                                            central_ProcessingClass.RequestMediaKeyToAuthenticationServer(UserId);
                                            SendMsg = "{ \"cmd\" : \"exchange_media_key\",\"result\":\"775a002021307577a964bc68845373f44841b1a7\",\"status\":\"is_success\"}";
                                        }
                                        break;
                                    default:
                                        SendMsg = "Succesws!";
